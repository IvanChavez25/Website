...
export class BaselineWeightComponent {
  ...
  calculateNutritionalStatus(age: number, weight: number, gender: string) {
    if (gender === 'female') {
      if (age == 0) {
        if (weight <= 2.0) {
          this.baselineData.weightStatus = 'SUW';
        } else if (weight <= 2.3) {
          this.baselineData.weightStatus = 'UW';
        } else if (weight <= 4.2) {
          this.baselineData.weightStatus = 'N';
        } else {
          this.baselineData.weightStatus = 'OW';
        }
      } else if (age == 1) {
        if (weight <= 2.7) {
          this.baselineData.weightStatus = 'SUW';
        ...
        this.baselineData.weightStatus = 'Unknown';
      }
    } else {
      if (age == 0) {
        if (weight <= 2.1) {
  ...
  onBirthdayChange() {
    this.calculateAge(this.baselineData.birthday);
    this.calculateAgeInMonth(this.baselineData.birthday);
  }
  ...
  calculateAge(birthdate: string) {
    ...
    this.baselineData.age = age.toString();
  }
  ...
  calculateAgeInMonth(birthdate: string) {
    ...
    if (ageInMonth < 0) {
      ...
      this.baselineData.ageInMonth = ageInMonth.toString();
    }
  }
  ...
  private isValidbaselineData(): boolean {
    return (
      ...
      (this.baselineData.nbsDone &&
        ...
        (this.baselineData.age || this.baselineData.age === 0) &&
        this.baselineData.ageInMonth &&
        this.baselineData.weight &&
  ...
  getSelectedChildNameOfHouseholdHead() {
    const selectedChildName = this.baselineData.nameOfChild;
    ...
    if (selectedChild) {
      this.baselineData.NameOfHouseholdHead = selectedChild.NameOfHouseholdHead;
      return selectedChild.NameOfHouseholdHead;
    } else {
      this.baselineData.NameOfHouseholdHead = '';
      return '';
    }

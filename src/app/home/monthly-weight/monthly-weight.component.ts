...
export class MonthlyWeightComponent {
  ...
  calculateNutritionalStatus(age: number, weight: number, gender: string) {
    if (gender === 'female') {
      if (age == 0) {
        if (weight <= 2.0) {
          this.monthlyWeightRecordData.weightForAge = 'SUW';
        } else if (weight <= 2.3) {
          this.monthlyWeightRecordData.weightForAge = 'UW';
        } else if (weight <= 4.2) {
          this.monthlyWeightRecordData.weightForAge = 'N';
        } else {
          this.monthlyWeightRecordData.weightForAge = 'OW';
        }
      } else if (age == 1) {
        if (weight <= 2.7) {
          this.monthlyWeightRecordData.weightForAge = 'SUW';
        ...
        this.monthlyWeightRecordData.weightForAge = 'Unknown';
      }
    } else {
      if (age == 0) {
        if (weight <= 2.1) {
